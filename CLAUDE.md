...
- CARETAKER role → allow-list `CARETAKER_PATHS` (`/expenses`, `/maintenance`, `/complaints`, `/utilities`, `/vendors`, `/help/tutorials`, plus `/select-org`, `/onboarding`, `/invite`); anything else redirects to `/maintenance`
- ADMIN / MANAGER / ACCOUNTANT → full access

Manager-only routes (OWNER is blocked) per `src/middleware.ts`: `/income`, `/expenses`, `/petty-cash`, `/tenants`, `/settings`, `/arrears`, `/recurring-expenses`, `/import`, `/insurance`, `/assets`, `/maintenance`, `/complaints`, `/utilities`, `/airbnb`, `/forecast`, `/vendors`, `/cases`, `/automations`. (Note: `/compliance`, `/asset-maintenance`, `/calendar`, `/billing`, `/upgrade` are reachable by all authenticated roles.)
...
- **Petty cash**: may tick `paidFromPettyCash`; the linked OUT row is created **PENDING** (`resolvePettyCashOutStatus` in `src/lib/petty-cash-status.ts`) and the float holder confirms on `/petty-cash` (`notifyPettyCashPending` emails them). **The expense path now applies the same repair-authority threshold as the Petty Cash page for every role**: a manager's "paid from petty cash" expense above `ManagementAgreement.repairAuthorityLimit` also creates a PENDING row (the expense form shows an amber note; `paymentReference` is stored as the row's `receiptRef`, no receipt is *required* on this path). Editing a linked row re-evaluates only when the amount changes (`reevaluatePettyCashOutStatus`): crossing the limit un-approves it, dropping back under while PENDING approves it, a REJECTED row is left alone, and a description-only edit never touches approval. Once APPROVED the caretaker's expense is locked (409); a caretaker edit of a PENDING/REJECTED row resubmits it. **Rejecting** a linked OUT row reverts the expense to unpaid and emails the creator. The caretaker never sees a balance: `/petty-cash`, `GET /api/petty-cash`, dashboard, report, `/api/hints` and `/api/inbox` are all denied, and `hintTypesVisibleTo("CARETAKER")` (`src/lib/hint-visibility.ts`) suppresses `LOW_PETTY_CASH` in both hint surfaces for Phase 2.
...
- **Convert a portal message thread into a complaint** (manager-only): "Log as complaint" on the tenant page's Portal Msgs tab → `POST /api/tenants/[id]/messages/[threadId]/convert-to-complaint { category, title?, description? }`. The tenant becomes the complainant (source PORTAL, so it appears in their portal with the SLA running), their own messages become the description, the thread is linked one-way via `PortalMessageThread.complaintId` (409 on a second attempt; migration `20260904150000_portal_thread_complaint_link`) and gets a MANAGER reply pointing them to Request → Complaint. The chat thread itself stays open.
- **Meter readings** (shipped): `/utilities` Readings tab only — `GET /api/utilities/meters`, `GET/POST /api/utilities/readings`, `PATCH /api/utilities/readings/[id]` (own SUBMITTED rows). The caretaker payload carries **units only** — never rates, amounts, invoice links or anomalies (`canSeeUtilityMoney` in `src/lib/utility-readings.ts`). See "Utility Metering" below.
- Not built by decision: a caretaker inbox (`/complaints` is its own SLA-sorted queue).

### Tenant Complaints

A `TenantComplaint` row is the domain record (property, complainant `tenantId`, their `unitId`, the `subjectUnitId` it is *about*, `category`, `source` STAFF | PORTAL, `raisedByUserId` / `raisedByName`); a `CaseThread` with `caseType = COMPLAINT` and `subjectId = complaint.id` carries status, stage, SLA and the timeline — the same split as maintenance, minus a mirrored status column (read status through `caseThread`). Migration `20260904120000_tenant_complaints`.

- **Workflow** `COMPLAINT_V1` (`src/lib/case-workflow-defs.ts`): received (24 h, **overridden by `ManagementAgreement.kpiStandardResponseHrs`** in `computeDefaultStageSlaHours`) → acknowledged (72 h) → investigating (120 h) → awaiting_tenant (pauses the clock) → resolved → closed. `naturalCompletionIndex = 1`, so resolving from Acknowledged is COMPLETED_NORMALLY, never BYPASSED.
- **Actions, not advance/regress**: `PATCH /api/complaints/[id] { action, note? }` with `acknowledge | investigate | await_tenant | resolve` (ops staff incl. CARETAKER) and `reopen | close` (manager tier). Pure rules in `src/lib/complaint-rules.ts` (`decideComplaintAction`, forward-only, reopen needs a note and a terminal stage); server side in `src/lib/complaints.ts` (`createComplaint`, `applyComplaintAction`, `loadComplaintForSession`).
- **Categories** (`ComplaintCategory`, permanent): `NOISE`, `NEIGHBOUR`, `SECURITY`, `PREMISES`, `STAFF_CONDUCT`, `OTHER`. **`STAFF_CONDUCT` is manager-only end to end** because the caretaker may be the subject: excluded from the caretaker's list query (`complaintCategoryFilter`), 404 (not 403) on direct GET/PATCH/events, 403 `CATEGORY_NOT_ALLOWED` on create, absent from their form (`categoriesSelectableBy`).
- **Timeline** goes through `src/lib/case-events.ts` (`parseCaseEventRequest` + `appendCaseEvent`), shared with `POST /api/cases/[id]/events`, which adds the limits the case route lacked: ≤ 8 files, ≤ 10 MB, images + PDF, body ≤ 20 000 chars; a storage failure is a 503 `STORAGE_UNAVAILABLE`, never a half-written event. Comments carry `meta.visibleToTenant` — **staff notes default to hidden**; the complainant's own description is always visible.
- **Notifications** (`src/lib/complaint-notify.ts`, two separate toggles because the audiences differ): `notifyNewComplaint` emails `getPropertyManagers` via `complaintRaisedTemplate` (`NOTIFY_NEW_COMPLAINT`, default on, per-user NOTIFICATION opt-out); `notifyComplaintResolved` emails the tenant of a PORTAL-sourced complaint via `complaintResolvedTemplate` with the resolution note (`NOTIFY_COMPLAINT_RESOLVED`, default on). Managers also see complaints in `/cases` (type filter "Complaint", "View as complaint" link) and the inbox's `CASE_NEEDS_ATTENTION` rows.
- **Resolution note**: the note passed with the `resolve` action is written as a tenant-visible COMMENT (`meta.resolution = true`) so the portal shows it, and goes into the tenant email.
- **Portal** (`src/app/api/portal/[token]/complaints/route.ts`, token auth, rate-limited 20/IP/hour): `POST { category, title, description }` creates the complaint with `source: PORTAL`, `tenantId` + `unitId` from the token, actor = the tenant by name (no user); `GET` lists rows where `tenantId = tenant AND source = PORTAL` — a complaint staff logged that names the tenant, or where they are only the subject unit's occupant, is never shown — with `updates` = COMMENT events flagged `visibleToTenant` (`byStaff` = has an `actorUserId`). Tenants may pick any category incl. `STAFF_CONDUCT`; the caretaker still never sees those.
- Fixed in passing: `POST /api/cases` now seeds `workflowKey` / `stageSlaHours` (manual cases were invisible to the SLA cron); `GET /api/cases?caseType=` validates instead of casting; `WORKFLOW_LENGTHS` is derived from `WORKFLOWS`.
- Smoke: `npx tsx scripts/caretaker-smoke.ts` covers the complaint matrix (147 checks incl. the STAFF_CONDUCT 404s).
...
### Income ↔ Invoice link
When a `LONGTERM_RENT` income entry is created via `POST /api/income`, the route auto-finds an open invoice for that tenant/month and marks it PAID in the same `prisma.$transaction`. Reverse: marking an invoice PAID via `PATCH /api/invoices/[id]` creates income entries if none exist (`invoiceId` on `IncomeEntry` prevents duplicates).

**Invoice lines** (migration `20260911090000_invoice_line_types`): besides `rentAmount / serviceCharge / otherCharges / lateFeeAmount`, a tenant invoice may carry the once-off move-in lines `depositAmount`, `leaseFee` (`totalAmount` = Σ all, `invoiceLinesTotal` in `src/lib/invoice-payment.ts`). **Several invoices per tenant per month are allowed** (the DB unique on `[tenantId, periodYear, periodMonth]` was replaced by an index); only ONE invoice with `rentAmount > 0` per period is enforced, in `POST /api/invoices` (409 names the existing rent invoice) and `generateInvoicesForTenants` (skips tenants with a non-cancelled rent invoice). `Property.leaseFeeDefault` prefills the form's **Move-in** preset. UI: `src/components/invoices/InvoiceForm.tsx` (presets Monthly rent / Move-in / Deposit only / Custom, add/remove lines, comma-formatted amounts, live total, duplicate-rent warning, tenants scoped to the selected property; also the edit form for non-PAID invoices), `InvoiceLineChips` on list rows, and `/invoices?new=move-in&tenantId=` deep link from the Tenants-page onboarding prompt.

**Paying an invoice always goes through the allocator** (`allocateInvoicePayment` in `src/lib/invoice-payment.ts`, Prisma side `buildInvoicePaymentOps` in `src/lib/invoice-payment-entries.ts`): the amount fills the rent side (rent + service charge + other + late fee → ONE `LONGTERM_RENT` entry, as before — the rent ledger counts only that type), then **water, then electricity** (`UTILITY_RECOVERY` entries tagged `IncomeEntry.utilityType` — see "Utility Metering"), then `DEPOSIT`, then `LEASE_FEE`, net of earlier payments; a short payment leaves the tail unpaid (`invoiceOutstandingByBucket(inv, paid)` says which lines). An overpayment folds into the last NON-utility bucket (a rent credit on a utilities-only invoice) — never into a utility line. Marking an invoice PAID over earlier part payments books the remainder (PATCH, bulk-mark-paid, verify-proof) instead of booking nothing. Used by `PATCH /api/invoices/[id]`, `bulk-mark-paid`, `verify-proof`, `reconcile/confirm`, and `POST /api/income` (a manual `LONGTERM_RENT` or `UTILITY_RECOVERY` entry against an invoice with ANY non-rent line — `invoiceHasNonRentLines`: utilities, deposit, lease fee — is split the same way and the response carries `allocation`; the auto-match prefers the rent invoice over a utilities-only invoice of the same month). `LEASE_FEE` is **landlord income** (`ADMIN_FEE` is a retired enum value nothing writes) (in gross income / owner statement "other income" / mgmt-fee base); `DEPOSIT` stays excluded. The tenant statement subtracts `depositAmount` from the invoice charge (the deposit has its own block). `unpay` deletes every linked entry.
...
Every payment a tenant makes (rent, deposit, fees) has a **receipt PDF** (`src/lib/receipt-pdf.tsx`) built from a payment event — the group of income entries created together, derived by `receiptGroupKey` (same `invoiceId` + same calendar day, else the entry itself) in `src/lib/payment-receipt.ts` (pure: group key, primary entry, `RCPT-YYYYMMDD-XXXXXX` number derived from the primary entry id, per-type line labels, PAID IN FULL vs PART PAYMENT stamp at the 1 % settle tolerance). `loadPaymentReceipt(entryId)` in `src/lib/payment-receipt-data.ts` assembles the data (invoice position, deposit position) — shared by `GET /api/portal/[token]/payments/[entryId]/receipt` (tenant), `GET /api/income/[id]/receipt` (manager), `POST /api/income/[id]/receipt/email` (manual send) and the automatic send. The legacy `GET /api/portal/[token]/invoices/[invoiceId]/receipt` URL delegates to the invoice's latest payment.

**Automatic receipt**: `maybeAutoEmailReceipt` in `src/lib/receipt-email.ts` runs (awaited, never throws) after the payment-recording sites above create entries — gated by the `TENANT_PAYMENT_RECEIPTS` NOTIFICATION automation (default **on**; per-property override; `resetAutomationCache()` first so a stale toggle is never served), skipped for tenants without an email, logged to the tenant's Comms tab as `templateUsed: "RECEIPT"`. One receipt per payment event — an invoice flipped PAID against pre-existing entries does not resend. Importers / demo seed / owner-invoice mark-paid never email tenants. Portal surfaces: Overview deposit row (receipt per deposit payment), Balance timeline (`PAYMENT_RECEIVED` events are collapsed per payment event by the ledger route and carry `receiptUrl` + `lines`), Files tab (generated receipts listed first). Manager surfaces: Download / Email receipt on PAID invoice rows (`GET /api/invoices` includes the latest `incomeEntries[0].id`), the tenant Deposit tab's receipt list, and Income page rows.

### Letting fee

`calcLettingFee` in `src/lib/letting-fee.ts`: the agreement's `newLettingFeeRate` (default 50 %) applies to the tenant's **full first-month charge — rent + service charge + parking fee**, never rent alone. `POST /api/owner-invoices/generate-letting-fee` accepts an optional `tenantId` (one tenant, idempotent per tenant via the `refTenantId` on the line — two new tenants in one month get two invoices; the period mode keeps its per-period 409). Line items keep `tenantId: null` (the fee is owner income; `PATCH /api/owner-invoices/[id]` creates an entry per line with `tenantId`). The Tenants-page onboarding prompt shows the breakdown and calls this route; it also links to the move-in invoice.

### Lease preparation fee recovery

The tenant's lease agreement fee (`LEASE_FEE` income entry) is paid into the landlord's account, so the manager recovers it from the owner. `POST /api/owner-invoices/generate-mgmt-fee` appends one pass-through line per `LEASE_FEE` entry received in the period that no non-cancelled owner invoice already recovers (`pendingLeaseFeeRecoveries` / `recoveryLineItem` in `src/lib/lease-fee-recovery.ts`); the line carries `refIncomeEntryId` (the tracking key), `isRecovery: true`, `tenantId: null`, and is added **after** the tax block (a reimbursement, not a fee). A management-fee invoice with only recovery lines is allowed. Removing the line from a draft makes the fee pending again. `GET /api/owner-invoices/lease-fee-recovery?propertyId=` returns collected / invoiced / settled / pending + rows (`leaseFeeRecoveryStatus`) for the panel on the Owner Invoices tab. `ownerInvoiceLineItemSchema` accepts `refTenantId` / `refIncomeEntryId` / `isRecovery` / `isTaxLine` so an edit of a generated invoice keeps the refs (the edit modal spreads the original line).
...
- **Management-fee base excludes `DEPOSIT` and `UTILITY_RECOVERY`** (`MGMT_FEE_EXCLUDED_INCOME_TYPES` / `mgmtFeeBase()` in `src/lib/management-fee.ts`) at every %-of-gross site: owner statement, both report builders, dashboard, `generate-mgmt-fee`. Utility recovery stays in gross income (the surplus is owner income); the manager just earns no percentage of a tenant's water / power bill. A new fee-base site must use `mgmtFeeBase`, never a bare `type !== "DEPOSIT"` filter.
...
`IncomeEntry` has a `type` field (`LONGTERM_RENT`, `AIRBNB`, `DEPOSIT`, `SERVICE_CHARGE`, `UTILITY_RECOVERY`, `OTHER`, the owner-fee types, and the once-off tenant charge `LEASE_FEE` — landlord income, excluded from the rent ledger; `ADMIN_FEE` is retired) and optional `checkIn`/`checkOut`, `nightlyRate`, and `platform` (`AIRBNB`, `BOOKING_COM`, `DIRECT`, `AGENT`) for Airbnb bookings.
...
- **Stable-ID round-trip (expenses and assets):** to let users edit *any* field without duplicating, the expenses template carries an optional **`ID`** column. `GET /api/import/expenses/export` downloads existing expenses pre-filled into the template (IDs populated); the `ImportSection` "Export existing" button (`onExportExisting` prop + `downloadRowsAsWorkbook` in import-templates.ts) drives this. In `mode: "upsert"` the route matches an `ID` to an accessible expense **before** the fingerprint and updates *every* field in place (`updatedIds` dedupes repeats; an unknown ID falls through to create with a warning). Replicate this pattern (ID column + export endpoint + ID-first match) for other importers that need true edit-and-resync.
...
Bank / M-Pesa details are typed in ONE place — Settings → Payment Accounts (`PaymentAccount`). The tenant form's "Payment account for invoices" (`TenantFormFields`, shown when the parent passes `unitAccounts`) writes the tenant's **unit override** `Unit.paymentAccountId` — `tenantSchema.paymentAccountId` is form-only, stripped by `POST/PUT /api/tenants` which validate it via `checkUnitPaymentAccount` (`src/lib/unit-payment-account.ts`) and update the unit in the same transaction (undefined = leave the unit alone; the form re-reads the unit's current override whenever the unit changes so a save never silently clears it). `PaymentAccountSelect` takes `inheritAccountId` to name the property default in the inherit option. The property form's "Payment account for tenant invoices" writes `ManagementAgreement.paymentAccountId` (via `POST/PATCH /api/properties` → agreement upsert; the property list returns `agreement.paymentAccountId`), the same field the agreement page edits. The agreement's **manager billing** uses `mgmtPaymentAccountId` (migration `20260911210000_payment_accounts_everywhere`, relation names `AgreementTenantAccount` / `AgreementMgmtAccount`); the owner-invoice PDF resolves that account first (bank/M-Pesa and PIN) and falls back to the legacy inline `mgmt*` columns. The agreement's `tenantKraPin` / `mgmtKraPin` inputs were removed from the page (PIN / VAT come from the payment account); the columns remain as read-only legacy fallbacks. The unused `Property.landlordEntity / bankName / bankAccountName / bankAccountNumber` columns were dropped.
...
**InsurancePolicy** — per-property insurance records (types: `BUILDING`, `PUBLIC_LIABILITY`, `CONTENTS`, `OTHER`) with premium frequency, coverage amounts, broker details, and document uploads. API: `GET/POST /api/insurance`, `GET/PATCH/DELETE /api/insurance/[id]`, `GET/POST /api/insurance/[id]/documents`, `DELETE …/documents/[docId]`. Documents (`InsurancePolicyDocument`) carry a `category` (`InsuranceDocumentCategory`: `POLICY_SCHEDULE`, `CERTIFICATE`, `VALUATION_REPORT`, `INSURER_ASSESSMENT`, `CLAIM`, `INVOICE_RECEIPT`, `OTHER`), an optional `documentDate` (the date *on* the document) and the uploader; migration `20260905090000_insurance_document_category`. **Storage**: new uploads go to the private `tenant-documents` bucket under `insurance/<policyId>/` and `fileUrl` holds the bucket *path*, signed on read by `withInsuranceDocumentUrls` (`src/lib/insurance-document-urls.ts`); rows from before 2026-09 hold a public URL in the old `property-documents` bucket and pass through (`isLegacyPublicUrl`). Storage failure on upload is a 503 `STORAGE_UNAVAILABLE`. Pure helpers (categories, allowed types, `policyLifecycle`, `renewalDates`) live in `src/lib/insurance-documents.ts` (unit-tested). UI: `InsuranceDocuments` (`src/components/insurance/`) is used both inside the add/edit modal (queues files until the POST returns an id, then `uploadAllTo`) and in the card's docs panel; the card shows category chips, flags Building/Contents policies without a `VALUATION_REPORT`, offers **Renew** (new policy, same cover, next term — the old one stays on record) and **Log premium** (`/expenses?prefill=insurance&…`, which the Expenses page turns into an INSURANCE-category entry). `GET /api/insurance` returns `documentsCount` + `documentCategories` per policy and the property's `currency`; PATCH verifies access to a new `propertyId` and that end > start.

**Asset Register** — asset inventory with serial numbers, warranty dates, purchase cost and `replacementValue` (Decimal, migration `20260905120000_asset_document_category_replacement_value`). Linked to maintenance schedules (frequency-based) and maintenance logs; a log with a cost creates a `MAINTENANCE` expense (creator stamped, vendor carried) and is audited. API: `/api/assets`, `/api/assets/[id]`, `/api/assets/[id]/schedules`, `/api/assets/[id]/schedules/[scheduleId]/log`, `/api/assets/[id]/logs` (includes vendor). POST/PATCH verify the unit belongs to the property and PATCH checks access to a new `propertyId`; the list returns `documentsCount`, `documentCategories` and the property's `currency`. **Documents** (`AssetDocument`, `AssetDocumentCategory`: `WARRANTY`, `MANUAL`, `INVOICE_RECEIPT`, `SERVICE_REPORT`, `CERTIFICATE`, `PHOTO`, `OTHER`, plus `documentDate` and uploader) use the same private-storage pattern as insurance: `src/lib/document-files.ts` (allowed types, storage paths, legacy public-URL detection), `src/lib/entity-document-urls.ts` (`withSignedDocumentUrls`, `removeStoredDocumentFile`), and the shared `EntityDocuments` uploader (`src/components/documents/`) wrapped as `AssetDocuments` / `InsuranceDocuments` — used in the add/edit modal (queue → `uploadAllTo` after POST) and the card's docs panel. `warrantyLifecycle` in `src/lib/asset-documents.ts` drives the badge, the warranty filter and the `WARRANTY_EXPIRY` calendar event (Building group, never overdue). **Disposal**: `Asset.disposedAt` + `disposalNotes` (migration `20260905150000_asset_disposal_warranty_alerts`, which also adds the `WARRANTY_EXPIRY_*` enum labels) — set/cleared via PATCH (`disposedAt`, not in the future; null reinstates). Disposed assets stay with their history but are excluded from `GET /api/assets` unless `?includeDisposed=true`, from the page KPIs/banners, the calendar, the warranty cron and the contents-cover total. **Contents cover**: `GET /api/insurance` groups in-service assets' `replacementValue` per property and attaches `contentsCheck` (`src/lib/contents-cover.ts`, `contentsCoverCheck` — `ok` / `under` with shortfall / `no_cover_figure` / `no_assets`, 2% tolerance) to CONTENTS policies; the insurance card renders it. **Import**: `/import` Assets tab → `POST /api/import/assets` (fingerprint property+name+serial, `mode: "upsert"`, ID-first match, soft unit/vendor misses, `createMany` + chunked updates) and `GET /api/import/assets/export` for the ID round-trip.
...
**Aggregator**: `buildCalendarEvents(propertyIds, from, to)` in [src/lib/calendar-events.ts](src/lib/calendar-events.ts) — same shape as `buildInbox()`: one `Promise.all`, reads only, no request state. Every calendar surface goes through it, so the in-app view and the ICS feed can't drift. Thirteen `EventType`s: `LEASE_EXPIRY`, `LEASE_START`, `RENT_DUE` (Invoice.dueDate, excludes DRAFT/CANCELLED), `MAINTENANCE_DUE` (recurring schedule), `MAINTENANCE_VISIT` (`MaintenanceJob.scheduledDate`, excludes DONE/CANCELLED), `INSURANCE_RENEWAL`, `WARRANTY_EXPIRY` (`Asset.warrantyExpiry`; informational, never overdue), `COMPLIANCE_EXPIRY`, `RECURRING_EXPENSE`, `RENT_REMITTANCE`, `MGMT_FEE_INVOICE`, `APPROVAL_DEADLINE` (`ApprovalRequest.expiresAt`), `CASE_SLA` (computed via `computeCaseSlaDueDate`, filtered in memory since it isn't a column).
...
**Recipients** — `getPropertyManagers(propertyId, orgId)` returns `{ userId, email, name }[]` = org-admins + managers with `PropertyAccess`, **falling back to the org's contact email** (`Organization.email`, `userId: null`) when no manager is found, so alerts are never silently dropped. The predicate is `propertyManagerWhere` in `src/lib/manager-recipients.ts` and keys on the **membership** role (`UserOrganizationMembership.role`), never the global `User.role` (an invited admin keeps a global role of MANAGER) — every recipient lookup uses it: cron alerts, complaint / petty-cash notifications, the three portal notify routes, case auto-assignment and the `/api/automations` recipient display. ACCOUNTANT / OWNER / CARETAKER are never recipients. `GET /api/automations` also resolves and returns the per-property recipient list for the UI's "Notifies N recipients" / "no recipients" display.
...
- Existing email-paired: `INVOICE_OVERDUE`, `LEASE_EXPIRY_30D`, `LEASE_EXPIRY_7D`, `URGENT_OPEN_4H`, `COMPLIANCE_EXPIRY_*`, `INSURANCE_EXPIRY_*`, `WARRANTY_EXPIRY_*`
...
### Utility Metering (water / electricity)

Post-paid only (no prepaid tokens, by decision). Page `/utilities` (tabs: Readings · Review & bill · Paid & unpaid · Reconciliation · Meters & tariffs; a CARETAKER only ever gets Readings). Migration `20260918090000_utility_metering`. Pure rules in `src/lib/utility-billing.ts`, Prisma side `src/lib/utility-readings.ts`, smoke `npx tsx scripts/utilities-smoke.ts` (97 checks against `npm run dev`).

- **Models**: `UtilityMeter` (`utility` WATER | ELECTRICITY, `role` UNIT | COMMON | BULK, several per unit — hot water is a second WATER meter with `ratePerUnitOverride`; `unitId` null for COMMON/BULK; `openingReading`), `UtilityTariff` (time-sliced per property + utility: `effectiveFrom`, `supplyRate`, `fuelRate` — tenant rate = supply + fuel; water never takes a fuel rate; `resolveTariffForPeriod` picks the latest on or before the period's last day), `UtilitySetting` (`unitLabel`, `requirePhoto`, `holdInvoicesForReadings`), `MeterReading` (`SUBMITTED → APPROVED`, `VOID`; rate + amount + occupant `tenantId` snapshotted at approval; `photoPaths[]`). **Only UNIT meters are ever priced or billed**; BULK (KPLC) and COMMON readings feed the reconciliation.
- **"Billed" is derived, never stored**: `invoiceId` set AND that invoice not CANCELLED. Deleting or cancelling an invoice releases its readings for re-billing with no extra write (both FKs are `SetNull`); a CANCELLED invoice that carried utilities can therefore never be un-cancelled (400).
- **Invariants** (`utility-readings.ts`): `previousReading` is server-derived (latest non-VOID reading of an earlier period, else the opening reading) — a manager override needs `previousOverrideReason`; one non-VOID reading per meter per period is enforced in code (index, not unique — a VOID row is reused); only a meter's LATEST reading may be edited or voided; `Invoice.waterAmount` / `electricityAmount` always equal the attached readings, are never typed (read-only in `InvoiceForm`, absent from the invoice zod schemas, preserved — not read from the body — by the PATCH total recompute) and never change once money is booked (`canChangeInvoiceUtilities`: status DRAFT/SENT/OVERDUE, no `paidAmount`, no income entries). Consumption is rounded to 3 dp (48.70 − 46.80), amounts to 2.
- **Billing**: readings of month M go on the invoice of month M+1 (strictly-earlier periods, stragglers included). `generateInvoicesForTenants` attaches them as it creates the rent invoice (nested `meterReadings: { connect }`); `billApprovedReadings` (`POST /api/utilities/bill`) merges into the tenant's changeable invoice of the period (rent invoice preferred; returned in `merged` — re-send if it was SENT) or raises a utilities-only invoice (`rentAmount 0`). Vacant-unit readings (`tenantId` null) are never billed. The `AUTO_INVOICE_GENERATION` cron holds tenants whose unit has an active unit meter without an APPROVED prior-month reading until the 5th (`unitsAwaitingReadings`), filtered BEFORE generation and NOT written to `AutomationExecution`; stragglers after that are billed by the manager's action, never silently by the cron.
- **Invoice PDF** (`src/lib/invoice-utility-lines.ts`, shared by the manager and portal PDF routes): one line per reading via `readingLineLabel` ("Jun 26 Water: 3 units (Prev: 176.00, Curr: 179.00) @ 175.00"); the arrears box is net of part payments and split rent / water / electricity. The late-fee base excludes unpaid utilities.
- **Paid & unpaid** (`src/lib/utility-statement.ts` pure + `utility-statement-data.ts`): per tenant, water / electricity billed · paid · unpaid from each invoice's payment walk, oldest unpaid period, approved-not-invoiced, last utility payment. `GET /api/utilities/statement` (`&format=pdf`), Excel `exportUtilityStatement`, `POST /api/utilities/statement/remind` (`src/lib/utility-reminder.ts` — always the all-time position, logged to Comms as `utility_reminder`; tenants without an email are reported as failures). Same loader serves `GET /api/tenants/[id]/utilities` (tenant detail "Utilities" tab + tenant-mode PDF) and `GET /api/portal/[token]/utilities` — **APPROVED readings of the token's own tenant only**.
- **Reconciliation** (`src/lib/utility-reconciliation.ts` pure; `GET /api/utilities/reconciliation?propertyId=&year=`): water = collected − `WATER` expenses (council) = borehole surplus to owner; electricity = collected − `ELECTRICITY` expenses (KPLC) − `GENERATOR` expenses (fuel), plus tariff set-aside (units × supply / fuel rate) vs paid, bulk vs billed vs vacant vs common vs **awaiting-approval** kWh (SUBMITTED readings are metered, unpriced — never counted as unaccounted loss) and a cost-per-unit rate guide. Billed follows the READING month; collected / paid follow the CASH date (expense `amount + vatAmount`) — the year-to-date row is the honest one. Owner statements carry a utilities **memo** (`OwnerStatement.utilities`) that re-presents figures already in gross income / expenses — never a second deduction.
- **Photos**: `src/lib/meter-photos.ts` — ≤ 3 images, ≤ 10 MB, private `tenant-documents` bucket under `meter-readings/<meterId>/`, signed on read; upload is all-or-nothing and a storage failure is a 503 `STORAGE_UNAVAILABLE` (never a reading pointing at half its photos). Client compression is shared with the expense uploader (`src/lib/image-compress.ts`).
- **Inbox**: type `METER_READINGS`, one row per property and kind — readings awaiting approval (URGENT after the 5th), approved readings not on an invoice, meters not read for last month — deep-linking `/utilities?tab=&propertyId=&month=previous`.
- **Tax**: `UTILITY_RECOVERY` matches one `TaxConfiguration.appliesTo` rule — water and electricity cannot carry different VAT treatment yet.
- Demo data: `seedDemoUtilities` (`src/lib/demo-utilities.ts`) on the **Kilimani Court** demo shows the feature working end to end — per-unit water + electricity meters, KPLC bulk + common-area meters, tariffs, three months of readings billed onto the demo's existing rent invoices (totals, `paidAmount` and tagged `UTILITY_RECOVERY` receipts adjusted), a paid / rent-only / part-paid / arrears mix for the Paid & unpaid statement and the portal, two units' last readings left SUBMITTED (one a flagged spike) for the Review tab, and council / KPLC / generator expenses sized for a positive surplus. A Kilimani Court seeded before metering is topped up in place by `POST /api/demo/seed` (`utilitiesAdded: true`; `demoUtilitiesState` refuses once anyone has entered their own readings) — the Utilities page offers it as "Load sample meters & readings".
