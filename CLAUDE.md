...
- Not built by decision: a caretaker inbox (`/complaints` is its own SLA-sorted queue). Phase 2b (portal complaints) and 2c (caretaker search) are planned.
...
- **Notification**: `notifyNewComplaint` (`src/lib/complaint-notify.ts`) emails `getPropertyManagers` via `complaintRaisedTemplate`, gated by the `NOTIFY_NEW_COMPLAINT` automation (default on) + per-user NOTIFICATION opt-out. Managers also see complaints in `/cases` (type filter "Complaint", "View as complaint" link) and the inbox's `CASE_NEEDS_ATTENTION` rows.
