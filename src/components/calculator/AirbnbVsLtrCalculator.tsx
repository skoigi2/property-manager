  ...
  return (
    <span className="relative inline-flex group/tip align-middle print:hidden">
      <button
        ...
        tabIndex={-1}
        aria-label={text}
        className="w-4 h-4 rounded-full bg-gray-200 dark:bg-white/10 text-gray-500 dark:text-gray-400 text-[10px] font-semibold leading-none flex items-center justify-center cursor-help"
      >
        i
      </button>
      <span
        role="tooltip"
        className="pointer-events-none absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-56 rounded-lg bg-header dark:bg-black/90 text-white text-xs leading-relaxed px-3 py-2 opacity-0 group-hover/tip:opacity-100 group-focus-within/tip:opacity-100 transition-opacity duration-150 z-30 shadow-lg"
      >
        {text}
      </span>
    </span>
...
function NumberField({ id, label, tooltip, value, onChange, prefix, suffix, max }: FieldProps) {
  ...
  return (
    <div className="flex flex-col gap-1">
      <label htmlFor={id} className="flex items-center gap-1.5 text-xs font-medium text-gray-600 dark:text-gray-300">
        <span>{label}</span>
        <InfoTip text={tooltip} />
      </label>
      <div className="relative">
        {prefix && (
          <span className="absolute left-3 top-1/2 -translate-y-1/2 text-xs text-gray-400 dark:text-gray-500 pointer-events-none">
            {prefix}
          </span>
        )}
        <input
          ...
          className={`w-full rounded-lg border border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 text-sm text-header dark:text-white py-2 ${prefix ? "pl-10" : "pl-3"} ${suffix ? "pr-9" : "pr-3"} focus:outline-none focus:ring-2 focus:ring-gold/50 focus:border-gold transition-shadow`}
        />
        {suffix && (
...
function ResultRow({ label, value, bold, negative }: { label: string; value: string; bold?: boolean; negative?: boolean }) {
  return (
    <div className="flex items-center justify-between py-1.5">
      <dt className={`text-sm ${bold ? "font-semibold text-header dark:text-white" : "text-gray-500 dark:text-gray-400"}`}>{label}</dt>
      <dd className={`text-sm font-mono tabular-nums ${negative ? "text-expense" : bold ? "font-semibold text-header dark:text-white" : "text-gray-700 dark:text-gray-200"}`}>
        {value}
      </dd>
    </div>
  );
...
export function AirbnbVsLtrCalculator() {
  ...
  return (
    <div className="relative" id="calculator">
      ...
      {/* toast */}
      {toast && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 bg-header text-white text-sm px-5 py-3 rounded-xl shadow-lg animate-[fadeIn_.2s_ease-out] print:hidden" role="status">
          {toast}
        </div>
      )}
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6 print:hidden">
        <label className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-300">
          ...
          <select
            ...
            className="rounded-lg border border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 text-sm text-header dark:text-white py-2 px-3 focus:outline-none focus:ring-2 focus:ring-gold/50"
          >
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button onClick={handleShare} className="text-xs font-medium border border-gray-200 dark:border-white/15 text-gray-600 dark:text-gray-300 rounded-lg px-3.5 py-2 hover:border-gold hover:text-gold-dark dark:hover:text-gold transition-colors">
            Share results
          </button>
          <button onClick={handleCopyResults} className="text-xs font-medium border border-gray-200 dark:border-white/15 text-gray-600 dark:text-gray-300 rounded-lg px-3.5 py-2 hover:border-gold hover:text-gold-dark dark:hover:text-gold transition-colors">
            Copy results
          </button>
          <button onClick={() => window.print()} className="text-xs font-medium border border-gray-200 dark:border-white/15 text-gray-600 dark:text-gray-300 rounded-lg px-3.5 py-2 hover:border-gold hover:text-gold-dark dark:hover:text-gold transition-colors">
            Print
          </button>
          <button onClick={handleReset} className="text-xs font-medium border border-gray-200 dark:border-white/15 text-gray-600 dark:text-gray-300 rounded-lg px-3.5 py-2 hover:border-expense hover:text-expense transition-colors">
            Reset calculator
          </button>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-[1fr,380px] gap-8 items-start">
        ...
        <div className="space-y-6 min-w-0">
          ...
          <SectionCard>
            <h3 className="font-display text-lg text-header dark:text-white mb-1">Long-Term Rental Assumptions</h3>
            <p className="text-xs text-gray-400 dark:text-gray-500 mb-4">A 12-month tenancy at a fixed monthly rent.</p>
            ...
            <button type="button" onClick={() => setShowLtAdvanced((s) => !s)} className="mt-4 text-xs font-medium text-gold-dark dark:text-gold hover:underline print:hidden">
          ...
          {/* Airbnb assumptions */}
          <SectionCard>
            <h3 className="font-display text-lg text-header dark:text-white mb-1">Airbnb Assumptions</h3>
            <p className="text-xs text-gray-400 dark:text-gray-500 mb-4">
              Short-term letting is a separate business with its own cost structure — don&apos;t estimate it from monthly rent.
            </p>
            <p className="text-[11px] uppercase tracking-wide font-semibold text-gray-400 dark:text-gray-500 mb-3">Revenue</p>
            ...
            <p className="text-[11px] uppercase tracking-wide font-semibold text-gray-400 dark:text-gray-500 mt-6 mb-3">Operating costs</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <button type="button" onClick={() => setShowAbAdvanced((s) => !s)} className="mt-4 text-xs font-medium text-gold-dark dark:text-gold hover:underline print:hidden">
          ...
          {/* Hassle premium */}
          <SectionCard>
            <h3 className="font-display text-lg text-header dark:text-white mb-1">
            ...
            <div className="flex items-center gap-4">
              ...
              />
              <span className="font-mono text-sm font-semibold text-header dark:text-white whitespace-nowrap w-32 text-right">
                {fmt(hassle)}/mo
              </span>
            </div>
        ...
        <aside className="lg:sticky lg:top-24 space-y-4 min-w-0" aria-label="Comparison results">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className={`rounded-2xl border-2 p-4 transition-colors duration-300 ${ltWins ? "border-gold bg-gold/5" : "border-gray-100 dark:border-white/10 bg-white dark:bg-white/[0.04]"}`}>
              <p className="text-[11px] uppercase tracking-wide font-semibold text-gray-400 dark:text-gray-500">Long-term</p>
              <p className="font-mono text-xl sm:text-2xl font-bold text-header dark:text-white mt-1 tabular-nums break-words">{fmt(result.longTerm.monthlyNoi)}</p>
              <p className="text-[11px] text-gray-400 dark:text-gray-500">net per month</p>
              {ltWins && <p className="mt-2 text-[11px] font-semibold text-gold-dark dark:text-gold">★ Recommended</p>}
            </div>
            <div className={`rounded-2xl border-2 p-4 transition-colors duration-300 ${abWins ? "border-gold bg-gold/5" : "border-gray-100 dark:border-white/10 bg-white dark:bg-white/[0.04]"}`}>
              <p className="text-[11px] uppercase tracking-wide font-semibold text-gray-400 dark:text-gray-500">Airbnb</p>
              <p className="font-mono text-xl sm:text-2xl font-bold text-header dark:text-white mt-1 tabular-nums break-words">{fmt(result.airbnb.monthlyNoi)}</p>
              <p className="text-[11px] text-gray-400 dark:text-gray-500">net per month</p>
              {abWins && <p className="mt-2 text-[11px] font-semibold text-gold-dark dark:text-gold">★ Recommended</p>}
            </div>
          </div>

          <SectionCard className="!p-5">
            <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">Airbnb vs long-term difference</p>
            <p className={`font-mono text-lg font-bold tabular-nums ${monthlyDiff >= 0 ? "text-income" : "text-expense"}`}>
              {monthlyDiff >= 0 ? "+" : "−"}{fmt(Math.abs(monthlyDiff))}/month
            </p>

            <hr className="my-4 border-gray-100 dark:border-white/10" />

            <p className="text-[11px] uppercase tracking-wide font-semibold text-gray-400 dark:text-gray-500 mb-1">Long-term rental</p>
            <dl>
            ...
            <p className="text-[11px] uppercase tracking-wide font-semibold text-gray-400 dark:text-gray-500 mb-1">Airbnb</p>
            <dl>
      ...
      <section aria-labelledby="breakeven-heading" className="mt-14">
        <div className="rounded-3xl bg-header dark:bg-[#091525] text-white p-7 sm:p-12 relative overflow-hidden">
          ...
          <p className="text-gold text-xs font-semibold uppercase tracking-widest mb-3">The number that decides it</p>
          <h2 id="breakeven-heading" className="font-display text-3xl sm:text-4xl mb-6">Airbnb Breakeven Occupancy</h2>

          {breakeven !== null ? (
            <>
              <p className="font-mono text-6xl sm:text-7xl font-bold text-gold tabular-nums">{breakeven.toFixed(0)}%</p>
              <p className="text-gray-300 text-base sm:text-lg mt-4 max-w-2xl leading-relaxed">
            ...
            <div className="max-w-2xl">
              <p className="font-mono text-5xl sm:text-6xl font-bold text-gold">N/A</p>
              <p className="text-gray-300 text-base sm:text-lg mt-4 leading-relaxed">{interpretation.headline}</p>
      ...
      <section aria-labelledby="ops-heading" className="mt-12">
        <h2 id="ops-heading" className="font-display text-2xl sm:text-3xl text-header dark:text-white mb-2">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            ...
            <div key={i} className="bg-white dark:bg-white/[0.04] border border-gray-100 dark:border-white/10 rounded-2xl p-5 text-center">
              <p className="font-mono text-2xl sm:text-3xl font-bold text-header dark:text-white tabular-nums">{s.n}</p>
        ...
        <div className="flex flex-wrap gap-2 mt-3">
          {["Ongoing guest communications", "Continuous pricing & availability management"].map((t) => (
            <span key={t} className="text-xs text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-white/5 rounded-full px-4 py-2">{t}</span>
          ))}
        </div>
        <p className="font-display text-xl sm:text-2xl text-header dark:text-white mt-8">
      ...
      <section aria-labelledby="verdict-heading" className="mt-12">
        <SectionCard className="!p-7 sm:!p-9 border-2 !border-gold/40">
          <p className="text-gold-dark dark:text-gold text-xs font-semibold uppercase tracking-widest mb-2">Our hassle-adjusted recommendation</p>
          {result.verdict === "LONG_TERM_WINS" && (
            <>
              <h2 id="verdict-heading" className="font-display text-2xl sm:text-3xl text-header dark:text-white">Long-Term Rental Wins</h2>
              <p className="text-sm text-gray-600 dark:text-gray-300 mt-3 max-w-2xl leading-relaxed">
                Long-term renting produces similar profits with substantially lower operational complexity.
              </p>
              <ul className="mt-5 grid sm:grid-cols-2 gap-2 text-sm text-gray-600 dark:text-gray-300 max-w-2xl">
          ...
          {result.verdict === "AIRBNB_WINS" && (
            <>
              <h2 id="verdict-heading" className="font-display text-2xl sm:text-3xl text-header dark:text-white">Airbnb Wins Convincingly</h2>
              <p className="text-sm text-gray-600 dark:text-gray-300 mt-3 max-w-2xl leading-relaxed">
          ...
          {result.verdict === "TOO_CLOSE" && (
            <>
              <h2 id="verdict-heading" className="font-display text-2xl sm:text-3xl text-header dark:text-white">Too Close to Call</h2>
              <p className="text-sm text-gray-600 dark:text-gray-300 mt-3 max-w-2xl leading-relaxed">
      ...
      <section aria-labelledby="stress-heading" className="mt-12">
        <h2 id="stress-heading" className="font-display text-2xl sm:text-3xl text-header dark:text-white mb-2">Stress Test Analysis</h2>
        <p className="text-sm text-gray-500 dark:text-gray-400 mb-6 max-w-2xl">
        ...
        <div className="md:hidden space-y-3">
          {result.stressTests.map((s) => (
            <div key={s.key} className={`rounded-xl border p-4 ${s.diffVsLongTerm < 0 ? "border-expense/30 bg-expense/5" : "border-gray-100 dark:border-white/10 bg-white dark:bg-white/[0.04]"}`}>
              <p className="text-sm font-medium text-header dark:text-white">{s.label}</p>
              <div className="flex justify-between mt-2 text-sm">
                <span className="text-gray-400 dark:text-gray-500 text-xs">Airbnb NOI</span>
                <span className="font-mono tabular-nums text-gray-700 dark:text-gray-200">{fmt(s.airbnbAnnualNoi)}</span>
              </div>
              <div className="flex justify-between mt-1 text-sm">
                <span className="text-gray-400 dark:text-gray-500 text-xs">vs long-term</span>
                <span className={`font-mono tabular-nums font-semibold ${s.diffVsLongTerm < 0 ? "text-expense" : "text-income"}`}>
        ...
        <div className="hidden md:block overflow-x-auto rounded-2xl border border-gray-100 dark:border-white/10">
          <table className="w-full text-sm bg-white dark:bg-white/[0.04]">
            <thead>
            ...
            <tbody>
              {result.stressTests.map((s) => (
                <tr key={s.key} className={`border-t border-gray-100 dark:border-white/10 ${s.diffVsLongTerm < 0 ? "bg-expense/5" : ""}`}>
                  ...
                  <td className="px-5 py-3 text-right font-mono tabular-nums text-gray-700 dark:text-gray-200">{fmt(s.airbnbAnnualNoi)}</td>
                  <td className={`px-5 py-3 text-right font-mono tabular-nums font-semibold ${s.diffVsLongTerm < 0 ? "text-expense" : "text-income"}`}>
                    {s.diffVsLongTerm >= 0 ? "+" : "−"}{fmt(Math.abs(s.diffVsLongTerm))}
                  </td>
                </tr>
              ))}
        ...
        {breakeven !== null && (
          <p className="text-sm text-gray-600 dark:text-gray-300 mt-5 bg-gold/10 border border-gold/30 rounded-xl px-5 py-4 max-w-3xl">
      ...
      <section aria-labelledby="report-heading" className="mt-14 print:hidden">
        <div className="rounded-3xl border-2 border-gold/40 bg-gradient-to-br from-gold/10 via-transparent to-transparent p-7 sm:p-10">
          <div className="grid lg:grid-cols-2 gap-8 items-center">
            <div>
              <h2 id="report-heading" className="font-display text-2xl sm:text-3xl text-header dark:text-white">Want the Full Investment Analysis?</h2>
              <p className="text-sm text-gray-600 dark:text-gray-300 mt-2">Get a personalised investor report delivered to your inbox.</p>
            ...
            <div>
              {leadStatus === "sent" ? (
                <div className="bg-white dark:bg-white/[0.04] border border-gray-100 dark:border-white/10 rounded-2xl p-7 text-center">
                  ...
                  <h3 className="font-display text-xl text-header dark:text-white">Your report is on its way</h3>
                ...
                <form onSubmit={handleLeadSubmit} className="bg-white dark:bg-white/[0.04] border border-gray-100 dark:border-white/10 rounded-2xl p-6 sm:p-7 space-y-4">
                  <div className="flex flex-col gap-1">
                    <label htmlFor="lead-name" className="text-xs font-medium text-gray-600 dark:text-gray-300">First name</label>
                    <input
                      id="lead-name" type="text" required maxLength={80} value={firstName}
                      onChange={(e) => setFirstName(e.target.value)}
                      className="rounded-lg border border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 text-sm text-header dark:text-white py-2.5 px-3 focus:outline-none focus:ring-2 focus:ring-gold/50"
                      placeholder="Jane"
                    />
                  </div>
                  <div className="flex flex-col gap-1">
                    <label htmlFor="lead-email" className="text-xs font-medium text-gray-600 dark:text-gray-300">Email address</label>
                    <input
                  ...
                  {leadStatus === "error" && <p className="text-xs text-expense">{leadError}</p>}
                  <button
                    type="submit"
                    disabled={leadStatus === "sending"}
                    className="w-full bg-header dark:bg-gold text-white dark:text-header text-sm font-semibold py-3 rounded-xl hover:bg-header/90 dark:hover:bg-gold/90 transition-colors disabled:opacity-60"
                  >
      ...
      <section aria-labelledby="gwpm-heading" className="mt-14 print:hidden">
        <h2 id="gwpm-heading" className="font-display text-2xl sm:text-3xl text-header dark:text-white text-center max-w-3xl mx-auto">
          You Know Which Strategy Fits Your Investment Goals. Now Run It Efficiently.
        </h2>
        <p className="text-sm text-gray-500 dark:text-gray-400 text-center mt-3 max-w-2xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-4 mt-8 max-w-3xl mx-auto">
          <div className={`rounded-2xl border p-6 ${ltWins ? "border-gold bg-gold/5" : "border-gray-100 dark:border-white/10 bg-white dark:bg-white/[0.04]"}`}>
            <h3 className="font-display text-lg text-header dark:text-white mb-3">For long-term rentals</h3>
          ...
          <div className={`rounded-2xl border p-6 ${abWins ? "border-gold bg-gold/5" : "border-gray-100 dark:border-white/10 bg-white dark:bg-white/[0.04]"}`}>
            <h3 className="font-display text-lg text-header dark:text-white mb-3">For Airbnb &amp; short lets</h3>
            <ul className="space-y-2 text-sm text-gray-600 dark:text-gray-300">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-3 mt-8">
          <Link href="/examples" className="w-full sm:w-auto text-center bg-header dark:bg-gold text-white dark:text-header text-sm font-semibold px-8 py-3.5 rounded-xl hover:bg-header/90 dark:hover:bg-gold/90 transition-colors">
            See How GroundWorkPM Works
          </Link>
          <Link href="/signup" className="w-full sm:w-auto text-center border border-gray-300 dark:border-white/20 text-header dark:text-white text-sm font-semibold px-8 py-3.5 rounded-xl hover:border-gold hover:text-gold-dark dark:hover:text-gold transition-colors">
            Start Free
          </Link>
        </div>
      </section>
