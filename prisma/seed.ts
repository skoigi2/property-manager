...
async function main() {
  ...
  await prisma.propertyAccess.upsert({
    where: { userId_propertyId: { userId: manager.id, propertyId: mayfairSuites.id } },
    create: { userId: manager.id, propertyId: mayfairSuites.id },
    update: {},
  });
  ...
  // Mayfair Suites units (5 units: 2× ONE_BED, 2× TWO_BED, 1× THREE_BED)
  const unitM101 = await prisma.unit.create({
    data: { unitNumber: "M101", propertyId: mayfairSuites.id, type: UnitType.ONE_BED, monthlyRent: null, status: UnitStatus.ACTIVE },
  });
  const unitM102 = await prisma.unit.create({
    data: { unitNumber: "M102", propertyId: mayfairSuites.id, type: UnitType.ONE_BED, monthlyRent: null, status: UnitStatus.ACTIVE },
  });
  const unitM201 = await prisma.unit.create({
    data: { unitNumber: "M201", propertyId: mayfairSuites.id, type: UnitType.TWO_BED, monthlyRent: null, status: UnitStatus.ACTIVE },
  });
  const unitM202 = await prisma.unit.create({
    data: { unitNumber: "M202", propertyId: mayfairSuites.id, type: UnitType.TWO_BED, monthlyRent: null, status: UnitStatus.ACTIVE },
  });
  const unitM301 = await prisma.unit.create({
    data: { unitNumber: "M301", propertyId: mayfairSuites.id, type: UnitType.THREE_BED, monthlyRent: null, status: UnitStatus.ACTIVE },
  });
  ...
  // ── MAYFAIR SUITES — JANUARY 2026 ────────────────────────────────────────
  // M101 (1-bed): 3 bookings, ~20 booked nights
  ...
  // ─── MAYFAIR SUITES EXPENSES (Jan–Mar 2026) ──────────────────────────────
  const mayfairMonths = [
  ...
  for (const { date, month } of mayfairMonths) {
    await prisma.expenseEntry.createMany({
      data: [
        // M101 & M102 — 1-bed fixed costs
  ...
  // Variable electricity & consumables (Mayfair Suites)
  await prisma.expenseEntry.createMany({
    data: [
      // January
  ...
  // Capital / sunk costs (Mayfair Suites)
  await prisma.expenseEntry.createMany({
    data: [
  ...
  // Management fees (Mayfair Suites — 10% of gross monthly revenue)
  await prisma.expenseEntry.createMany({
    data: [
  ...
  // Mayfair Suites petty cash (Jan–Mar 2026)
  await prisma.pettyCash.createMany({
    data: [
