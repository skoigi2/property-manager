...
type PortalData = {
  ...
  /** DEPOSIT receipts linked to the tenant, newest first. */
...
export default function PortalPage({ params }: { params: { token: string } }) {
  ...
  useEffect(() => {
    ...
    if ((tab === "balance" || tab === "documents") && !ledger) loadLedger();
  ...
  // Receipts generated by the system for every payment (rent, deposit, fees).
  const generatedReceipts = (ledger?.events ?? []).filter(
    (e): e is Extract<LedgerEvent, { kind: "PAYMENT_RECEIVED" }> => e.kind === "PAYMENT_RECEIVED",
  );
  const docGroups = {
      ...
      <div className="max-w-lg mx-auto px-4 py-6 space-y-4">
        ...
        {tab === "overview" && (
          <>
            ...
            <div className="bg-white rounded-xl border border-gray-200 divide-y divide-gray-100">
              ...
              {(tenant.depositAmount > 0 || (data.depositReceipts?.length ?? 0) > 0) && (
                <div className="px-4 py-3">
                  <div className="flex justify-between items-center">
                    <span className="text-body text-gray-500">Security Deposit</span>
                    <span className="text-body font-semibold text-gray-900">
                      {data.depositReceived != null
                        ? `${formatCurrency(data.depositReceived, currency)} received`
                        : `${formatCurrency(tenant.depositAmount, currency)} due`}
                    </span>
                  </div>
                  {data.depositReceived != null && data.depositReceived < tenant.depositAmount && (
                    <p className="text-caption text-amber-700 mt-0.5">
                      {formatCurrency(tenant.depositAmount - data.depositReceived, currency)} of the {formatCurrency(tenant.depositAmount, currency)} deposit still to pay
                    </p>
                  )}
                  {(data.depositReceipts?.length ?? 0) > 0 && (
                    <div className="mt-1.5 flex flex-wrap gap-x-3 gap-y-1">
                      {data.depositReceipts!.map((r) => (
                        <a
                          key={r.id}
                          href={r.receiptUrl}
                          target="_blank"
                          rel="noreferrer"
                          className="text-caption text-green-700 hover:underline font-medium"
                        >
                          Receipt · {format(new Date(r.date), "d MMM yyyy")} · {formatCurrency(r.amount, currency)}
                        </a>
                      ))}
                    </div>
                  )}
                          ...
                          {e.kind === "INVOICE_ISSUED" ? (
                            ...
                            <>
                              <p className="text-body text-green-700">
                                {e.isDeposit ? "Deposit received" : "Payment received"}{e.invoiceNumber ? ` for ${e.invoiceNumber}` : ""}
                              </p>
                              <p className="text-caption text-gray-400">
                                {format(new Date(e.date), "d MMM yyyy")}
                                {e.paymentMethod ? ` · ${PAYMENT_METHOD_LABEL[e.paymentMethod] ?? e.paymentMethod}` : ""}
                                {e.lines.length > 1 ? ` · ${e.lines.map((l) => `${l.label} ${formatCurrency(l.amount, currency)}`).join(" + ")}` : ""}
                              </p>
                              <a
                                href={e.receiptUrl}
                                target="_blank"
                                rel="noreferrer"
                                className="inline-block text-caption text-green-700 hover:underline font-medium mt-0.5"
                              >
        ...
        {tab === "documents" && (
          <>
            {docsLoading ? (
            ...
            ) : docs.length === 0 && generatedReceipts.length === 0 ? (
              ...
              <>
                {generatedReceipts.length > 0 && (
                  <div>
                    <h2 className="text-label font-semibold text-gray-500 uppercase mb-2">Payment Receipts</h2>
                    <div className="space-y-2">
                      {generatedReceipts.map((r) => (
                        <div key={r.incomeEntryId} className="bg-white rounded-xl border border-gray-200 px-4 py-3 flex items-center gap-3">
                          <div className="text-h1 shrink-0">🧾</div>
                          <div className="min-w-0 flex-1">
                            <div className="text-body font-medium text-gray-900 truncate">
                              Receipt {r.receiptNumber}
                            </div>
                            <div className="text-caption text-gray-400 truncate">
                              {format(new Date(r.date), "d MMM yyyy")} · {formatCurrency(r.amount, currency)}
                              {r.lines.length > 1 ? ` · ${r.lines.map((l) => l.label).join(" + ")}` : ` · ${r.lines[0]?.label ?? "Payment"}`}
                            </div>
                          </div>
                          <a
                            href={r.receiptUrl}
                ...
                {([
                  ...
                  { key: "receipts", title: generatedReceipts.length > 0 ? "Payment Proofs You Uploaded" : "Payment Receipts", items: docGroups.receipts },
