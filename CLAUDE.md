...
| `calculations.ts` | `calcUnitSummary`, `calcPettyCashBalance`, `calcPettyCashTotal`, `calcOccupancyRate`, `calcLateInterest`, `calcExpensePayment` (derived paid/outstanding/status per expense). Constant: `RIARA_MGMT_FEE` (flat per unit type — legacy fallback used only by report builders/owner statements) |
...
- **Management fee**: owner-invoice generation (`POST /api/owner-invoices/generate-mgmt-fee`) computes LONGTERM fees from per-unit `ManagementFeeConfig` rows (`flatAmount`, else `ratePercent% × monthlyRent`; a unit with no config gets **no** fee line) and AIRBNB fees as 10% × cash income received in the period. `RIARA_MGMT_FEE` in `calculations.ts` survives only as a hardcoded expense-side fallback in the report builders / owner statements (`calcManagementFee` and `ALBA_MGMT_FEE_RATE` were dead code and have been deleted). **Known caveat**: owner statements and report P&L hardcode these legacy fee assumptions (RIARA flat fees by unit type for any LONGTERM property, flat 10% for any AIRBNB property) instead of reading `ManagementAgreement`/`ManagementFeeConfig` — a property with no fee arrangement still shows a fee there.
