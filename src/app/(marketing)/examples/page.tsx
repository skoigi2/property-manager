...
function SectionHeading({ kicker, title, body }: { kicker: string; title: string; body: string }) {
  return (
    <div className="max-w-2xl mx-auto text-center mb-10">
      <p className="text-label font-semibold uppercase text-gold mb-2">{kicker}</p>
      <h2 className=" text-h1 text-header dark:text-white mb-3">{title}</h2>
  ...
  return (
    <div className="max-w-lg mx-auto bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
      <div className="bg-header px-6 py-4 flex items-center justify-between">
        <div>
          <p className="text-white text-body font-medium">Owner Statement</p>
          <p className="text-white/50 text-caption ">Riverside Apartments</p>
        </div>
        <p className="text-gold text-caption font-semibold">May 2026</p>
      </div>
      <div className="px-6 py-4">
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.unit} className="border-t border-gray-50">
                ...
                <td className="py-2 text-right tabular-nums text-gray-700">{r.due}</td>
                <td className={`py-2 text-right tabular-nums ${r.ok ? "text-green-600" : "text-red-500"}`}>
                  {r.received}
                </td>
              </tr>
            ))}
        ...
        <div className="mt-4 bg-cream rounded-xl px-4 py-3 flex items-center justify-between">
          <p className="text-caption font-semibold text-header">Net payable to owner</p>
          <p className="tabular-nums text-body font-semibold text-green-700">KSh 312,400</p>
        </div>
        <p className="text-caption text-gray-300 mt-3 text-center">
  ...
  return (
    <div className="max-w-xs mx-auto bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden">
      <div className="bg-header px-5 py-4">
        <p className="text-white/50 text-label uppercase ">Tenant portal</p>
        <p className="text-white text-body font-medium mt-0.5">Hi Pauline 👋</p>
      </div>
      <div className="p-5 space-y-3">
        <div className="bg-red-50 border border-red-100 rounded-xl px-4 py-3">
          <p className="text-label text-red-400 uppercase ">Balance due</p>
          <p className="tabular-nums text-h3 text-red-600">KSh 60,000</p>
          <p className="text-caption text-gray-400 mt-0.5">Invoice INV-2026-0142 · due 5 Jun</p>
        </div>
        <button className="w-full bg-header text-white text-caption font-medium rounded-xl py-2.5">
          Submit payment proof
        </button>
        <div className="grid grid-cols-3 gap-2 text-center">
          {["Balance", "Receipts", "Repairs"].map((t) => (
            <div key={t} className="bg-cream rounded-lg py-2 text-caption text-gray-500">{t}</div>
          ))}
        </div>
        <p className="text-caption text-gray-300 text-center">
  ...
  return (
    <div className="py-16 px-4 space-y-24">
      ...
      <div className="max-w-3xl mx-auto text-center">
        ...
        <p className="text-body-lg text-gray-500 dark:text-gray-400 max-w-xl mx-auto">
      ...
      <div className="text-center">
        <Link
          href="/signup"
          className="inline-block bg-header text-white font-medium px-8 py-3.5 rounded-xl hover:bg-header/90 transition-colors"
        >
