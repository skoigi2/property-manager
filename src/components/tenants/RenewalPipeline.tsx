    ...
    <div className="space-y-6">
          ...
          return (
            <div key={stage.value} className="flex items-center flex-1">
              <div className="flex flex-col items-center flex-1 gap-1">
                ...
                <p className={clsx("text-caption text-center ", passed ? "text-income font-medium" : "text-gray-400")}>
                  {stage.label}
                </p>
              </div>
              {i < STAGES.length - 1 && (
      ...
      {!isRenewed && (
        <div className="bg-cream rounded-xl p-4 space-y-3">
          <p className="text-label text-gray-400 uppercase font-medium">Proposed Terms</p>

          {/* Escalation rate + auto-suggest */}
          <div className="flex items-end gap-3 p-3 bg-white border border-gold/20 rounded-lg">
            <div className="flex-1">
              <label className="text-caption text-gray-500 flex items-center gap-1">
            ...
            {suggestedRent && (
              <div className="text-right shrink-0">
                <p className="text-caption text-gray-400 ">Suggested rent</p>
                <p className="text-body tabular-nums font-semibold text-gold">{fmt(suggestedRent)}</p>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-caption text-gray-500 ">New Monthly Rent</label>
              <input
                type="number"
            ...
            <div>
              <label className="text-caption text-gray-500 ">New Lease End Date</label>
              <input
                type="date"
                value={editLeaseEnd}
          ...
          <div>
            <label className="text-caption text-gray-500 ">Notes</label>
            <textarea
              value={editNotes}
      ...
      {isRenewed && (
        <div className="bg-green-50 border border-green-100 rounded-xl p-4 space-y-2">
          <p className="text-body font-medium text-income flex items-center gap-2">
            <CheckCircle2 size={16} /> Lease renewed successfully
          </p>
          {proposedRent && (
            <p className="text-caption text-gray-600 ">
              New rent: <span className="tabular-nums font-medium">{fmt(proposedRent)}</span>
            </p>
          )}
          {proposedLeaseEnd && (
            <p className="text-caption text-gray-600 ">
          ...
          {renewalNotes && (
            <p className="text-caption text-gray-500 italic">{renewalNotes}</p>
          )}
        </div>
      )}
      ...
      <div className="flex items-center gap-2 flex-wrap">
        {currentStage === "NONE" && (
          <button
            ...
            disabled={saving}
            className="flex items-center gap-1.5 px-4 py-2 bg-gold text-white text-body rounded-lg hover:bg-gold-dark disabled:opacity-50 transition-colors"
          >
        ...
        {currentStage === "TERMS_AGREED" && (
          <button
            ...
            className="flex items-center gap-1.5 px-4 py-2 bg-income text-white text-body rounded-lg hover:bg-green-700 disabled:opacity-50 transition-colors"
          >
        ...
        {!isRenewed && currentStage !== "NONE" && (
          <button
            ...
            disabled={saving}
            className="flex items-center gap-1.5 px-4 py-2 bg-gray-100 text-gray-600 text-body rounded-lg hover:bg-gray-200 disabled:opacity-50 transition-colors"
          >
        ...
        {currentStage !== "NONE" && (
          <button
            onClick={reset}
            disabled={saving}
            className="flex items-center gap-1.5 px-3 py-2 text-gray-400 hover:text-expense text-caption rounded-lg transition-colors ml-auto"
          >
      ...
      <div className="pt-2 border-t border-gray-100">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-label text-gray-400 uppercase font-medium">
            Rent History
          </h3>
          <button
        ...
        {showAddEntry && (
          <div className="mb-3 p-3 bg-cream rounded-xl space-y-2">
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="text-caption text-gray-500 ">Monthly Rent</label>
                <input
                  type="number"
                  value={newEntryRent}
                  onChange={(e) => setNewEntryRent(e.target.value)}
                  className="mt-0.5 w-full border border-gray-200 rounded-lg px-2.5 py-1.5 text-body tabular-nums focus:outline-none focus:ring-2 focus:ring-gold/40"
                />
              </div>
              <div>
                <label className="text-caption text-gray-500 ">Effective Date</label>
                <input
                  type="date"
                  value={newEntryDate}
                  onChange={(e) => setNewEntryDate(e.target.value)}
                  className="mt-0.5 w-full border border-gray-200 rounded-lg px-2.5 py-1.5 text-body focus:outline-none focus:ring-2 focus:ring-gold/40"
                />
              </div>
            </div>
            <input
              ...
              className="w-full border border-gray-200 rounded-lg px-2.5 py-1.5 text-body focus:outline-none focus:ring-2 focus:ring-gold/40"
            />
            <div className="flex gap-2">
              <button
                onClick={addHistoryEntry}
                disabled={addingSaving || !newEntryRent || !newEntryDate}
                className="px-3 py-1.5 bg-gold text-white text-caption rounded-lg hover:bg-gold-dark disabled:opacity-50 transition-colors"
              >
                {addingSaving ? <Loader2 size={12} className="animate-spin inline" /> : "Save"}
              </button>
              <button
                onClick={() => setShowAddEntry(false)}
                className="px-3 py-1.5 text-gray-500 text-caption rounded-lg hover:bg-gray-100 transition-colors"
              >
                Cancel
              </button>
            </div>
          ...
          <div className="space-y-0 border border-gray-100 rounded-xl overflow-hidden">
            {history.map((entry, i) => {
              ...
              return (
                ...
                >
                  <div>
                    <p className="text-body tabular-nums font-semibold text-header">
                      {fmt(entry.monthlyRent)}
                      {delta !== null && (
                        <span className={clsx("ml-2 text-caption ", delta >= 0 ? "text-income" : "text-expense")}>
