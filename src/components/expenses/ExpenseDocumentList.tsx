  ...
  if (documents.length === 0) {
    return (
      <div className="flex flex-col items-center py-6 gap-2 text-gray-400">
        ...
        <p className="text-sm font-sans">No receipts or documents attached yet</p>
      </div>
    );
  }
            ...
            <div key={doc.id} className="rounded-xl border border-gray-100 bg-white overflow-hidden group">
              ...
              >
                {doc.url && isImage(doc) ? (
                  ...
                  <div className="flex flex-col items-center gap-1 text-gold">
                    <FileText size={22} />
                    <span className="text-[10px] font-sans uppercase text-gray-400">
              ...
              <div className="p-2.5">
                <div className="flex items-center gap-1.5">
                  <p className="text-xs font-sans font-medium text-header truncate flex-1" title={doc.label}>{doc.label}</p>
                  <span className={clsx("text-[10px] px-1.5 py-0.5 rounded font-sans shrink-0", categoryColor(doc.category))}>
                    {categoryLabel(doc.category)}
                  </span>
                </div>
                <p className="text-[11px] text-gray-400 font-sans mt-0.5 truncate" title={doc.fileName}>
                  {format(new Date(doc.uploadedAt), "d MMM yyyy")}
                  {doc.fileSize ? ` · ${formatBytes(doc.fileSize)}` : ""}
                </p>
                {(doc.uploadedByName || doc.uploadedByEmail) && (
                  <p className="text-[11px] text-gray-400 font-sans truncate flex items-center gap-1" title={doc.uploadedByEmail ?? undefined}>
      ...
      {confirmDoc && (
        <div className="fixed inset-0 bg-black/50 z-[60] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl p-6">
            <h3 className="font-display text-lg text-header mb-2">Delete this file?</h3>
            <p className="text-sm text-gray-600 mb-1"><span className="font-medium">{confirmDoc.label}</span></p>
            <p className="text-xs text-gray-400 mb-5">{confirmDoc.fileName} — this cannot be undone.</p>
        ...
        >
          ...
          <div className="flex items-center justify-between px-4 py-3 text-white/80" onClick={(e) => e.stopPropagation()}>
            <div className="min-w-0">
              <p className="text-sm font-sans font-medium truncate">{current.label}</p>
          ...
          {viewable.length > 1 && (
            <>
              ...
              <p className="absolute bottom-3 left-1/2 -translate-x-1/2 text-xs text-white/50 font-sans">
