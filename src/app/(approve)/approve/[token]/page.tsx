...
export default function ApprovePage() {
  ...
  if (submitted) {
    return (
      <Centered>
        ...
        <p className="text-gray-400 text-body mt-3">{new Date(submitted.at).toLocaleString()}</p>
      </Centered>
    );
  }
  ...
  if (data.status === "EXPIRED") {
    return (
      <Centered>
        <h1 className=" text-h1 mb-2">Approval expired</h1>
  ...
  if (data.status !== "PENDING") {
    ...
    if (isDisputeFlow && (data.status === "APPROVED" || data.status === "REJECTED")) {
      return (
        <Centered>
          <h1 className=" text-h1 mb-2">Flag this approval</h1>
          ...
          <div className="space-y-3 text-left max-w-md mx-auto">
            <label className="block text-body">Your name</label>
            <input
              value={name}
    ...
    return (
      <Centered>
        <h1 className=" text-h1 mb-2">Already responded</h1>
        <p className="text-gray-700">This approval was {data.status.toLowerCase()} by <strong>{data.respondedByName ?? "someone"}</strong></p>
        {data.respondedAt && <p className="text-gray-400 text-body mt-2">on {new Date(data.respondedAt).toLocaleString()}.</p>}
      </Centered>
    );
  }
  ...
  return (
    <Centered wide>
      <h1 className=" text-h1 mb-1">Approval requested</h1>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-5 text-left mb-6">
        <p className="text-label uppercase text-gray-400 mb-1">Question</p>
        <p className="text-gray-900 whitespace-pre-wrap">{data.question}</p>

        {data.amount != null && (
          <div className="mt-4">
            <p className="text-label uppercase text-gray-400 mb-1">Amount</p>
            <p className="text-h1 text-gray-900">{formatCurrency(data.amount, data.currency ?? "USD")}</p>
          </div>
        )}

        <div className="mt-4 text-body text-gray-500">
          <p>{data.propertyName}{data.unitNumber ? ` · Unit ${data.unitNumber}` : ""}</p>
          <p>Case: {data.caseTitle}</p>
          <p className="text-caption text-gray-400 mt-2">Expires {new Date(data.expiresAt).toLocaleString()}</p>
        </div>
      </div>

      <div className="space-y-3 text-left">
        <label className="block text-body font-medium text-gray-700">Your name (for the record)</label>
        <input
          value={name}
          onChange={(e) => setName(e.target.value)}
          className="w-full border border-gray-200 rounded-lg px-3 py-2 text-body"
