...
type TypeConfig = {
  ...
  badge: string;    // Tailwind classes for the filter chip (active)
...
export default function CalendarPage() {
  ...
  // When navigating months, clear the day selection
  function prevMonth() {
  ...
  const leadingBlanks = getDay(startOfMonth(current)); // 0=Sun
  const trailingBlanks = (7 - ((days.length + leadingBlanks) % 7)) % 7;

  // Map day-of-month → filtered events for that day
  ...
  // Visible events for the list section
  const listEvents = useMemo(() => {
    const base = events.filter((e) => activeTypes.has(e.type as EventType));
    if (selectedDay !== null) {
      const dayStr = String(selectedDay).padStart(2, "0");
      const prefix = `${year}-${String(month).padStart(2, "0")}-${dayStr}`;
          ...
          <div className="grid grid-cols-1 xl:grid-cols-[1fr_380px] gap-4">
                    ...
                    >
                      ...
                      {dotTypes.length > 0 && (
                        <div className="flex flex-wrap gap-0.5 px-0.5">
                          ...
                          {dayEvents.length > dotTypes.length && (
            ...
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden flex flex-col">
              <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
                ...
                <span className="text-xs text-gray-400 font-sans">{listEvents.length} event{listEvents.length !== 1 ? "s" : ""}</span>
              </div>

              {listEvents.length === 0 ? (
                <div className="flex-1 flex flex-col items-center justify-center py-12 text-gray-400">
                  <p className="text-sm font-sans">No events{selectedDay !== null ? " on this day" : " this month"}</p>
                  {selectedDay !== null && (
                    <button
                      ...
                      <div key={e.id} className="px-4 py-3 hover:bg-gray-50 transition-colors">
                        <div className="flex items-start gap-2.5">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-gray-400 font-sans mt-0.5 truncate">{e.propertyName}{e.unitName ? ` · ${e.unitName}` : ""}</p>
                          </div>
