...
export function TrialCTA({ headline = "Ready to stop managing properties the hard way?" }: { headline?: string }) {
  return (
    <div className="not-prose my-10 rounded-2xl bg-header dark:bg-[#091525] p-8 text-center">
      <p className=" text-h2 text-white mb-2">{headline}</p>
      <p className="text-white/60 text-body mb-6">
        30-day free trial · No credit card required · Cancel anytime
      </p>
      <Link
        href="/signup"
        className="inline-block bg-gold text-header font-semibold text-body px-8 py-3 rounded-lg hover:bg-gold/90 transition-colors"
      >
        Start free trial →
      </Link>
    </div>
    ...
    <div className="min-h-screen">
      ...
      <div className="pt-28 pb-12 px-6 bg-cream dark:bg-[#0C1B2E]">
        <div className="max-w-3xl mx-auto">
          {/* Breadcrumb */}
          <nav className="flex items-center gap-2 text-caption text-gray-400 mb-6">
          ...
          {/* Category + title */}
          <span className={`inline-block text-caption font-semibold px-3 py-1 rounded-full mb-4 ${CATEGORY_COLORS[post.category]}`}>
            {post.category}
          </span>
          <h1 className=" text-h1 text-header dark:text-white mb-4">
            {post.title}
          </h1>

          {/* Meta strip */}
          <div className="flex items-center gap-4 text-body text-gray-400 ">
      ...
      <div className="bg-white dark:bg-[#0C1B2E] px-6 py-12">
        <div className="max-w-3xl mx-auto">
          <article className="prose prose-gray dark:prose-invert prose-headings:text-header dark:prose-headings:text-white prose-h2:text-h2 prose-h3:text-h3 prose-p:text-gray-600 dark:prose-p:text-gray-300 prose-li:text-gray-600 dark:prose-li:text-gray-300 prose-strong:text-header dark:prose-strong:text-white prose-a:text-header dark:prose-a:text-gold prose-a:no-underline hover:prose-a:underline max-w-none">
            {children}
          </article>
        </div>
      </div>
      ...
      <div className="bg-cream dark:bg-[#091525] border-t border-gray-100 dark:border-white/10 px-6 py-16 text-center">
        <div className="max-w-xl mx-auto">
          <h2 className=" text-h1 text-header dark:text-white mb-3">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <Link
              href="/signup"
              className="bg-header dark:bg-gold text-white dark:text-header font-semibold text-body px-8 py-3 rounded-lg hover:bg-header/90 dark:hover:bg-gold/90 transition-colors"
            >
              Start free trial
            </Link>
            <Link
              href="/pricing"
              className="border border-gray-200 dark:border-white/20 text-gray-600 dark:text-gray-300 font-semibold text-body px-8 py-3 rounded-lg hover:bg-gray-50 dark:hover:bg-white/5 transition-colors"
            >
              See pricing
            </Link>
          </div>
