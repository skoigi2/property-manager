...
export function InboxActions({ item, onActionComplete }: Props) {
  ...
  if (item.type === "CASE_NEEDS_ATTENTION") {
    actions.push({
      key: "open-case",
      label: "Open case",
      tip: "Opens the full case workspace — timeline, attachments, status.",
      icon: ExternalLink,
      onClick: () => { window.location.href = `/cases/${item.refId}`; },
    });
    actions.push({
      key: "case-reassign",
      label: "Reassign",
      tip: "Reassigns this case to another user (PATCH /api/cases/[id]).",
      icon: UserPlus,
      onClick: () => setModal("case-reassign"),
    });
    actions.push({
      key: "case-waiting-on",
      label: "Set waiting on",
      tip: "Updates who the case is waiting on (PATCH /api/cases/[id]).",
      icon: Clock,
      onClick: () => setModal("case-waiting-on"),
    });
  }
...
function CaseReassignModal({ item, onClose, onSaved }: { item: InboxItem; onClose: () => void; onSaved: () => void }) {
  ...
  async function save() {
    setSaving(true);
    try {
      const r = await fetch(`/api/cases/${item.refId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
...
function CaseWaitingOnModal({ item, onClose, onSaved }: { item: InboxItem; onClose: () => void; onSaved: () => void }) {
  const OPTIONS = ["MANAGER", "OWNER", "TENANT", "VENDOR", "NONE"] as const;
  const [waitingOn, setWaitingOn] = useState<typeof OPTIONS[number]>("NONE");
  const [saving, setSaving] = useState(false);
  ...
  return (
    <Modal open onClose={onClose} title="Set waiting on" size="sm">
      <div className="p-5 space-y-4">
        <div className="grid grid-cols-2 gap-2">
          {OPTIONS.map((o) => (
            <button
